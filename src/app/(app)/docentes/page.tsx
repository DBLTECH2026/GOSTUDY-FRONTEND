...
import { useDocentes } from '@/modules/personas/api';
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-bg-card border border-border rounded-md">
        ...
        {isLoading ? (
          ...
          <div className="hidden md:grid grid-cols-[1fr_220px_140px_120px_110px] gap-4 px-5 py-3 bg-bg-muted border-b border-border text-[11px] font-bold tracking-widest text-text-muted">
            <span>DOCENTE</span><span>EMAIL</span><span>ESPECIALIDAD</span><span>CÓDIGO</span><span>ESTADO</span>
          </div>
        )}
        {docentes.map((d) => (
          <div
            key={d.id}
            className="grid grid-cols-2 md:grid-cols-[1fr_220px_140px_120px_110px] gap-x-3 gap-y-1 md:gap-4 px-5 py-3 border-b border-border last:border-0 text-sm items-center"
          >
      ...
      <NuevoDocenteModal open={open} onClose={() => setOpen(false)} onCreated={reload} />
    </div>
  );
}
